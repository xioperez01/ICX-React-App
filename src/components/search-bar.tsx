      ...
      <div className={cn("relative w-full", className)}>
        ...
        {!!props?.value && (
          <div
            className={cn(
              // base
              "absolute pointer-events-auto cursor-pointer bottom-0 right-2 flex h-full items-center justify-center",
              // text color
              "text-primary dark:text-primary-foreground"
            )}
            onClick={() => props.onChange?.({ target: { value: "" } } as any)}
          >
            <X className="size-[1.125rem] shrink-0" aria-hidden="true" />
          </div>
        )}
      </div>
    );
